...
public sealed class ReviewRegressionTests : IDisposable
{
    ...
    public async Task BuildAgent_IneligibleLicense_IsNotServed()
    {
        ...
        var response = await client.GetAsync( "/Lease.ashx?user=alice&machine=buildagent-1f2e&product=Ultimate&version=2025.1.0" );
    ...
    {
        this.application.AddLicense( LicenseBuilder.Default().WithMinPostSharpVersion( new Version( 2099, 1, 0 ) ) );
    ...
    {
        this.application.AddLicense( LicenseBuilder.Default().WithValidTo( new DateTime( 2020, 1, 1, 0, 0, 0, DateTimeKind.Utc ) ) );
    ...
    public async Task UnlimitedButExpiredLicense_IsDeniedRatherThanFailing()
    {
        ...
        var response = await client.GetAsync( "/Lease.ashx?user=alice&machine=desktop-1&product=Ultimate&version=2025.1.0" );
