# Runs the test suite against PostgreSQL, in a container that carries the server.
#
# The suite runs on SQLite in the ordinary build, which needs no server. PostgreSQL is one of the two engines
...
$arguments = @{
    ...
    Command = 'bash tests/docker/PostgreSql/run.sh'
}
